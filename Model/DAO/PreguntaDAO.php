...
class PreguntaDAO {
    ...
    public function getPregunta($preguntaDTO) {
        ...
        $pregunta = [];
        try {
            $stmGetPregunta = $this->conexion->prepare("select pr.idpregunta,pr.dadouno,pr.dadodos,pr.dadotres,pr.dadocuatro,pr.numero,pr.fecharegistro,date_add(pr.fecharegistro, INTERVAL (3*60) second)fechafinal from pregunta pr INNER JOIN partido p on p.idpartido=pr.idpartido INNER JOIN estadopregunta ep on ep.idestadopregunta=pr.idestadopregunta WHERE ep.descripcion='activa'  and p.idpartido=:idpartido and pr.activo");
