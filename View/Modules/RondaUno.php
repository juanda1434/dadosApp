...
//                        imageDado.src = `View/Public/img/${d}.png`;
                ...
                $("#btnResponder").on("click", () => {
                    if (!estaRuleta) {
                        $.post("POST/EnviarRespuesta", getOperacion(), (r) => {
                            console.log(r);
                            if (r.exito != undefined && r.exito) {
                                if (r.respuesta) {
...
//                    }
                ...
                function actualizarCampo() {
                    $.get("GET/CampoActual/codigo=" + readGet.codigo, (r) => {
                        console.log(r);
                        if (r.opcion != undefined) {
                            switch (r.opcion) {
                                    ...
                                    if (!estaRuleta) {
                                        if (cronometroInterval != null) {
                                            $(document).Toasts('create', {
                                                class: 'bg-danger',
                                                title: 'Ten en cuenta!',
                                                body: 'Pasando a la siguiente pregunta!'
                                            });
                                            clearInterval(cronometroInterval);
                                            cronometroInterval = null;
                                        }
                                ...
                                case 3:
                                    if (r.puntajes != undefined) {
                                        ...
                                        let listPuntajes = `<li class="list-group-item">
                                                            <b>#</b> - <b>Nombre</b> <a class="float-right"><b>Puntaje</b></a>
                                                        </li>
                                                        `;
                                        let cantidadNiños = 0;
                                        for (var key in r.puntajes) {
                                            ...
                                            let mid = "";
                                            if (key < 16) {
                                                mid = r.id == rr[key].id ? "activoo win" : "";
                                            } else {
                                                mid = r.id == rr[key].id ? "activoo lose" : "";
                                            }

                                            cantidadNiños++;
                                            let pu = parseInt(key) + 1;
                                            listPuntajes += `<li class="list-group-item ${mid}">
                                                           ${pu} - ${rr[key].nombre} <a class="float-right">${rr[key].puntaje}</a>
                                                        </li>`;
                                        }
                                        $("#tablapuntuacionesd").removeClass("d-none");
                                ...
                                case 4:
                                document.location.href="InicioEstudiante";
                                    break;
                            }
